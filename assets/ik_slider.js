;(function ( $, window, document, undefined ) {
	...
	var pluginName = 'ik_slider',
		defaults = {
			'instructions': 'Use the left and right arrow keys to decrease or increase the slider value. ',
			'minValue': 0,
			'maxValue': 100,
			'nowValue': 0,
			'step': 1
		...
		if( !plugin.textfield.is(':text') ) {
			...
			plugin.knob = $('<div/>')
				.attr({
                    ...
                    'aria-labelledby': id + '_instructions' // add description; trying this instead of 'describedby' in an attempt to skirt a ChromeVox issue
				})
				.addClass('ik_knob')
